...
public static class Utility
{
		...
		else
		{
			...
			if (script is BasicAction)
			{
				...
				if (exportType == ExportType.PseudoCode)
					...
					export += indent(indentLevel) + "<action type=\"" + (script as BasicAction).actionType + ((script as BasicAction).actionType.Equals(BasicAction.ActionType.VarInt) || (script as BasicAction).actionType.Equals(BasicAction.ActionType.VarBool) || (script as BasicAction).actionType.Equals(BasicAction.ActionType.VarString) ? "\" val=\"" + (script as BasicAction).value : "") + "\"/>"+(script.GetComponent<CurrentAction>() ? "<!-- Current Action -->" : "") +"\n";
			}
			// Cas d'un CAPTOR
	...
	public static IEnumerator pulseItem(GameObject newItem)
	{
		...
		while (newItem != null  && newItem.transform.localScale.x > initScaleX) // newItem peut �tre nul si le GameObject est supprim� avant la fin de la coroutine
		{
		...
		if (newItem != null) // newItem peut �tre nul si le GameObject est supprim� avant la fin de la coroutine
			newItem.transform.localScale = new Vector3(1, 1, 1);
	}
