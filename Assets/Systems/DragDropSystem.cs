...
public class DragDropSystem : FSystem
{
	...
	// see inputFiels in VarInt prefab
	public void onlyInteger(GameObject varIntBlock, string newValue){
		...
		varIntBlock.GetComponent<BasicAction>().value = res.ToString();
		// compute context
		string context = exportEditableScriptToString(varIntBlock.GetComponentInParent<UIRootContainer>().transform, varIntBlock);

		// g�n�rer une trace seulement sur la scene principale
		if ((!res.Equals(oldValue)) && SceneManager.GetActiveScene().name == "MainScene")
		{
			GameObjectManager.addComponent<ActionPerformedForLRS>(varIntBlock, new
			{
				verb = "modified",
				objectType = "block",
				activityExtensions = new Dictionary<string, string>() {
	...
	// see inputFiels in VarBool prefab
	public void onlyBoolean(GameObject varBoolBlock, string newValue){
		string oldValue = varBoolBlock.GetComponent<BasicAction>().value;
		string res = "";
		...
		varBoolBlock.GetComponent<BasicAction>().value = res;
		// compute context
		string context = exportEditableScriptToString(varBoolBlock.GetComponentInParent<UIRootContainer>().transform, varBoolBlock);

		// g�n�rer une trace seulement sur la scene principale
		if (res != oldValue && SceneManager.GetActiveScene().name == "MainScene")
		{
			GameObjectManager.addComponent<ActionPerformedForLRS>(varBoolBlock, new
			{
				verb = "modified",
				objectType = "block",
				activityExtensions = new Dictionary<string, string>() {
	...
	// see inputFiels in VarString prefab
	public void onlyString(GameObject varStringBlock, string newValue){
		string oldValue = varStringBlock.GetComponent<BasicAction>().value;
		varStringBlock.GetComponent<BasicAction>().value = newValue;
		// compute context
		string context = exportEditableScriptToString(varStringBlock.GetComponentInParent<UIRootContainer>().transform, varStringBlock);

		// g�n�rer une trace seulement sur la scene principale
		if ((!newValue.Equals(oldValue)) && SceneManager.GetActiveScene().name == "MainScene")
		{
			GameObjectManager.addComponent<ActionPerformedForLRS>(varStringBlock, new
			{
				verb = "modified",
				objectType = "block",
				activityExtensions = new Dictionary<string, string>() {
