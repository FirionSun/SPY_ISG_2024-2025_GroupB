	...
	private GameObject readXMLInstruction(XmlNode actionNode)
	{
		...
		switch (actionNode.Name)
		{
			...
			case "action":
				...
				switch(type){
					case "VarInt":
						BaseElement actInt = obj.GetComponent<BasicAction>();
						((BasicAction)actInt).value = actionNode.Attributes.GetNamedItem("val").Value;
						obj.GetComponentInChildren<TMP_InputField>().text = ((BasicAction)actInt).value;
						break;
					case "VarBool":
						BaseElement actBool = obj.GetComponent<BasicAction>();
						((BasicAction)actBool).value = actionNode.Attributes.GetNamedItem("val").Value;
						obj.GetComponentInChildren<TMP_InputField>().text = ((BasicAction)actBool).value;
						break;
					case "VarString":
						BaseElement actString = obj.GetComponent<BasicAction>();
						((BasicAction)actString).value = actionNode.Attributes.GetNamedItem("val").Value;
						obj.GetComponentInChildren<TMP_InputField>().text = ((BasicAction)actString).value;
						break;
					default:
						break;
				}
